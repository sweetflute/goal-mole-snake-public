...
$(document).on("pagecreate", "#step3", function(event) {
	...
	$("#step3InitStartBtn").on("click", function(){
		...
		recordStartTime(1, parseInt(groupID), round, -1, -1);
	});
...
function updateStep3Score(score, level){
	$("#step3Score").html(score);
	$("#step3Level").html(level);
}

function showStep3Result(score, level) {
	// alert("showstep3Result:" + score + ":" + level);
	...
	var groupID = $.cookie("groupID");
	if(groupID == 1){
		$("#systemGoal1_1").html(score + 14);
	}else if (groupID == 2){
		$("#systemGoal2_1").html(score + 8);
		$("#systemGoal2_2").html(score + 14);
	}
}
...
$(document).on("pagecreate", "#step4", function(event) {
	...
	$("#step4InitStartBtn" + groupID).on("click", function(){
		...
		round ++;
		recordStartTime(1, parseInt(groupID), round, goal1, goal2);
...
function updateStep4Score(score, level){
	$("#step4Score").html(score);
	$("#step4Level").html(level);
}

function showStep4Result(score, level) {
	// alert("showstep4Result:" + score + ":" + level);
	var groupID = $.cookie("groupID");
